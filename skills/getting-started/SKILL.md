...
description: First-run setup for Recoup inside Claude. Walks a customer through email + PIN verification, issues an API key tied to their real account, persists it locally, looks up their org via the Recoup API, and seeds Claude's memory so future music-industry questions (streaming, campaigns, artists, releases, document syncing) route through the recoup-api skill. Use this skill the first time someone installs the Recoupable plugin, or when they say "set up Recoup", "set up Recoupable", "install Recoup", "get an API key", "connect to Recoup", "connect Claude to Recoup", "use Recoup", "onboard to Recoup", "how do I start", or "I just joined the Recoup enterprise deal".
---
...
Now that the key works, use the **`recoup-api` skill** (already installed as part of recoup-platform-plugin) to list the orgs this account belongs to. Defer the endpoint choice to that skill — it knows the docs at `https://developers.recoupable.com` and will pick the correct call.
...
try:
    ...
    pattern = re.compile(r"<!-- recoup-getting-started:start -->.*?<!-- recoup-getting-started:end -->\n?", re.DOTALL)
